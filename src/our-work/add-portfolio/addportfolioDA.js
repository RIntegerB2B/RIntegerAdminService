            ...
            if (err) {
                ...
                SuperCategory.find({
                    ...
                    if (err) {
                        ...
                        var subCat = mainCat.category.id(req.params.catid);
                        subCat.primeImage = appSetting.workServerPath + category[0].categoryName + '/' + mainCat.mainCategoryName + '/' + subCat.categoryName + '/' + 'prime' + '/' + subCat.primeImage;
                        var arr = subCat.images;
                        for( var i = 0; i<= arr.length -1; i++) {
            arr[i] = appSetting.workServerPath + category[0].categoryName + '/' + mainCat.mainCategoryName + '/' + subCat.categoryName + '/' + arr[i] ;
                        }
...
exports.deleteMulitpleImage = function (req, res) {
    SuperCategory.find({
        ...
        if (err) {
           ...
           const PATH = appSetting.workUplaodPath + category[0].categoryName + '/' + mainCat.mainCategoryName + '/' + subCat.categoryName + '/' + req.params.image; 
            fs.unlink(PATH, (err) => {
                if (err) {throw err;} 
                else {
                    var list = arr;
    ...
    , function (err, categoryDetails) {
        if (err) {
            ...
            var mainCat = categoryDetails.mainCategory.id(req.params.main);
             ...
             subcat.remove();
        const PATH =  appSetting.workUplaodPath + categoryDetails.categoryName + '/' + mainCat.mainCategoryName + '/' + subcatname;
                        ...
                        SuperCategory.find({
                            ...
                            if (err) {
                                ...
                                for (var i = 0; i <= arr.length - 1; i++) {
                                    ...
                                    categoryName: String,
                                    arr[i].primeImage = appSetting.workServerPath + category[0].categoryName + '/' + mainCat.mainCategoryName + '/' + arr[i].categoryName + '/' + 'prime' + '/' + arr[i].primeImage;
                                }
